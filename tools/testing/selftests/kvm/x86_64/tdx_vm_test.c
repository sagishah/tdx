...
void guest_io_reads(void)
{
	...
	ret = tdg_vp_vmcall_instruction_io(TDX_IO_READS_TEST_PORT, 1, PORT_READ,
					   &data);
	if (ret)
		tdx_test_fatal(ret);
	if (data != 0xAB)
	...
	ret = tdg_vp_vmcall_instruction_io(TDX_IO_READS_TEST_PORT, 2, PORT_READ,
					   &data);
	if (ret)
		tdx_test_fatal(ret);
	if (data != 0xABCD)
	...
	ret = tdg_vp_vmcall_instruction_io(TDX_IO_READS_TEST_PORT, 4, PORT_READ,
					   &data);
	if (ret)
		tdx_test_fatal(ret);
	if (data != 0xFFABCDEF)
	...
	ret = tdg_vp_vmcall_instruction_io(TDX_IO_READS_TEST_PORT, 5, PORT_READ,
					   &data);
	if (ret)
		tdx_test_fatal(ret);
...
void verify_host_reading_private_mem(void)
{
	...
	TDX_RUN(vcpu);
	TDX_TEST_ASSERT_IO(vcpu, TDX_HOST_READ_PRIVATE_MEM_PORT_TEST, 4,
			   PORT_WRITE);
...
#define TDX_MAX_VCPUS	512
	vm_enable_cap(vm, KVM_CAP_MAX_VCPUS, TDX_MAX_VCPUS);
	...
	for (i = 0; i < num_vcpus; i++) {
		...
		TEST_ASSERT_EQ(ret_max_vcpus, TDX_MAX_VCPUS);
		/* VCPU_INDEX = i */
		TEST_ASSERT_EQ(r9, i);
		/*
